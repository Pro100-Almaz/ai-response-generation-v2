...
lint: ## Run linting with ruff
	uv run ruff check src/ai_response_generation_v2/

lint-fix: ## Run linting with ruff and fix auto-fixable issues
	uv run ruff check --fix src/ai_response_generation_v2/

format: ## Format code with ruff
	uv run ruff format src/ai_response_generation_v2/

type-check: ## Run type checking with mypy
	uv run mypy src/ai_response_generation_v2/

check: ## Run all checks (lint + format check + type check)
	uv run ruff check src/ai_response_generation_v2/
	uv run ruff format --check src/ai_response_generation_v2/
	uv run mypy src/ai_response_generation_v2/
...
test-cov: ## Run tests with coverage
	uv run pytest tests/ -v --cov=src/ai_response_generation_v2 --cov-report=html --cov-report=term
